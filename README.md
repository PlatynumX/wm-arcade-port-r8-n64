...
- What death doesn't do yet. `wm_arcade_adjust_health` flips `player_mode` to `WM_PMODE_DEAD` and nothing else: no death sound, no wrestler-type death-animation dispatch (`wm_arcade_move_bret`'s own `WM_PMODE_DEAD` case already calls a `mode_dead` callback for this, left `NULL`), no `LIFEBAR.ASM` combo multiplier/drone-count/speed-adjustment damage scaling, no `LAST_DAMAGE` timestamp update, and -- the actual gap -- nothing reacts to a dead wrestler at the match level: there's still no win condition, no round-over state, no ring announcer. A human can now genuinely kill the CPU opponent (or vice versa) and the match just keeps ticking forever with one dead body standing in place.
