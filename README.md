...
- Hit detection itself. `wm_arcade_check_wrestler_collisions`/`wm_arcade_try_attack_hit` (already real and ctest-verified since fix38) are still not called from `wm_match`, and even wiring them wouldn't land a hit yet: the victim's `hurt_box` (`wm_arcade_set_hurt_box`) needs per-frame `ANI3` hit-box data (`SYS.EQU`'s `IANI3X`/`Y`/`Z`/`ID`) that's baked into the WIMP image assets themselves, not the animation script -- a third asset format this port hasn't parsed yet, distinct from both the frame-name (`wm_visual_sequence`) and typed-opcode (`wm_source_anim_state`) systems already in the tree. `wm_bret_backend_callbacks` also doesn't set `adjust_health`. A human can walk Bret around and throw the 6 wired attacks with a real, source-accurate active window, but nothing lands, no health changes, and there's no win condition.
