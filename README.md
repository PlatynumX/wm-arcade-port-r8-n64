...
- Each attack animation's own leading `ANI_SETFACING` (`HRTSEQ2.ASM`, e.g. `hrt_2_punch_anim`'s own instant `FACING_DIR=NEW_FACING_DIR` copy right as the punch starts) -- only the torso turn-transition sequences' `ANI_SETFACING` is wired so far. `ANI_XFLIP` inside the leg's own turn animations (a purely cosmetic sprite-mirror mid-turn) is also not translated -- no state this port tracks depends on it. `CAN_MOVE_DIR` ring-boundary redirection (`execute_walk`'s down-left/down-right special case) is also not translated, since it depends on the unported ring-boundary system.
