...
- **Real movement physics, and a real walk cycle to go with it.** `wm/movement.h` translates `WRESTLE.ASM::convert_facing` and `set_velocities` exactly (per-direction velocity table, `walk_fast`/opponent-ONGROUND-or-DEAD 1.5x boost, backward-facing 0.9x reduction) and wires `execute_walk`'s `MOVE_DIR`/`OBJ_CONTROL` (`M_FLIPH`) dispatch as Bret's real `execute_walk` callback. `wm/bret_backend.h` also now runs `change_walk_anim`'s leg-cycle half: all 12 `hrt_walkM_fF_anim` sequences `hrt_leg_anims_table` (`BRET.ASM:2897`) can select are extracted and wired, so a Bret actor with real input genuinely walks with the correct animation, not just a position change under a frozen sprite. `change_walk_anim`'s torso half (`hrt_torso_anims_table`, `BRET.ASM:2981`) is real now too, using both of the table's real indices -- see the next bullet for what feeds them.
- **`FACING_DIR`/`NEW_FACING_DIR` are real state now, not a substitute.** `wm/arcade/wm_arcade_closest.h`'s `wm_arcade_update_newfacing` translates `WRESTLE.ASM:3018`'s `update_newfacing` (live `NEW_FACING_DIR` toward the opponent, every tick, for every actor -- wired into `wm_match_tick` unconditionally, exactly where `WRESTLE.ASM`'s own per-process main loop calls it), and `wm/movement.h`'s `wm_execute_walk` translates `set_rotate_anim`'s literal `FACING_DIR=NEW_FACING_DIR` catch-up for the `#zip`/stance case (`WRESTLE.ASM:5082-5083`). Just like the source, `FACING_DIR` stays frozen at its last idle value while actually walking -- `change_walk_anim`'s leg half never writes it -- so facing one way while walking another (e.g. after running past a repositioned opponent) is now representable instead of impossible. Both actors also start the match with real `FACING_DIR`/`NEW_FACING_DIR` values from `WRESTLE.ASM:2753-2755`'s `#teamX_starts` placement table (`WM_MOVE_UP_RIGHT` for P1, `WM_MOVE_DOWN_LEFT` for the opponent) instead of a zeroed default. With both indices real, `change_walk_anim`'s torso half now genuinely reaches the off-diagonal (mid-turn) region of `hrt_torso_anims_table` -- it just doesn't have artwork there yet, see the boundary below -- while the diagonal region it always used to hit still reduces to picking between `hrt_torso2_anim` and `hrt_torso4_anim` (`hrt_torso8_anim`/`hrt_torso6_anim` turned out to be literal `SUBR` aliases of those two in `HRTSEQ1.ASM`, not separate artwork, confirmed by extracting and diffing their frame data). `set_rotate_anim`'s own turn-animation *selection* for the idle-turn case is still not translated -- see the boundary below.
...
- `hrt_torso_anims_table`'s 12 off-diagonal turn-transition entries (`hrt_2_to_4_turn2_anim` etc.) -- genuinely reachable now that `FACING_DIR` can lag `NEW_FACING_DIR` mid-walk, but the artwork itself isn't extracted, so `wm_bret_torso_anim` returns `NULL` for them and the last torso sprite just keeps playing. `set_rotate_anim`'s own turn-animation *selection* for the `#zip`/idle-turn case (`hrt_rotate_anims_table`'s 12 turn-transition sequences plus `hrt_stand6/8_anim`, and the `change_anim1` dispatch that would play them) is also not translated -- `FACING_DIR` updates for real the instant a wrestler goes idle, it just doesn't animate the turn yet. `CAN_MOVE_DIR` ring-boundary redirection (`execute_walk`'s down-left/down-right special case) is also not translated, since it depends on the unported ring-boundary system.
