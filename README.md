...
- What still keeps a human from landing a hit through ordinary play. Hit detection itself is wired now (see above), but `wm_arcade_move_bret`'s own `do_punch` (`BRET.ASM`) always treats the opponent as point-blank -- `near(a,50,45)`, since nothing in this port yet computes `closest_xdist`/`closest_zdist` -- so pressing punch currently selects the unwired `WM_BRET_ANIM_BUTT2/4` headbutt instead of a mapped punch id, and no attack anim (hence no `ATTACK_ON`) ever actually starts. `wm_bret_backend_callbacks` also doesn't set `adjust_health` (only `wm_match_tick`'s own collision wiring does, not `wm_arcade_move_bret`'s per-actor reaction callbacks such as `teammate_pin`/`can_pin`). There's still no win condition, no `MODE_DEAD` transition, and no LIFEBAR combo/drone-count/speed-adjustment damage scaling.
