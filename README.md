...
- The rest of the already-ported combat core's connection to a live match: `wm_arcade_move_ported_wrestler`'s generic 8-wrestler dispatch isn't used yet (only Bret is wired, directly); the other seven wrestlers have no visual backend at all. `DRONE.ASM`'s AI script/range data (`wnshort_t`/`wnmed_t`/`wnlong_t`, `blkbase_t`/`blkatk_t`/`sklhhdly_t`/`sklhrdly_t`) is emitted by a source macro (`SKLM`) that is not present in the checked-in tree, so it is not guessed -- the drone AI therefore never actually gives a wrestler a button/stick to act on. `WRESTLE.ASM::execute_walk` (movement velocity and idle-animation reselection, shared by every wrestler) is also not translated. Attract-mode wrestlers therefore hold real health/ring state and Bret can visibly stand in the ring correctly animated today, but nothing moves, walks, or fights yet.
