...
- **Real movement physics, and a real walk cycle to go with it.** `wm/movement.h` translates `WRESTLE.ASM::convert_facing` and `set_velocities` exactly (per-direction velocity table, `walk_fast`/opponent-ONGROUND-or-DEAD 1.5x boost, backward-facing 0.9x reduction) and wires `execute_walk`'s `MOVE_DIR`/`OBJ_CONTROL` (`M_FLIPH`) dispatch as Bret's real `execute_walk` callback. `wm/bret_backend.h` also now runs `change_walk_anim`'s leg-cycle half: all 12 `hrt_walkM_fF_anim` sequences `hrt_leg_anims_table` (`BRET.ASM:2897`) can select are extracted and wired, so a Bret actor with real input genuinely walks with the correct animation, not just a position change under a frozen sprite. `change_walk_anim`'s torso half (`hrt_torso_anims_table`, `BRET.ASM:2981`) is real now too: `wm_bret_torso_anim` translates its full 4x4 diagonal-index scheme, though real `FACING_DIR`/`NEW_FACING_DIR` tracking (see the boundary below) means every reachable call lands on the table's diagonal, which reduces to picking between `hrt_torso2_anim` and `hrt_torso4_anim` -- `hrt_torso8_anim`/`hrt_torso6_anim` turned out to be literal `SUBR` aliases of those two in `HRTSEQ1.ASM`, not separate artwork, confirmed by extracting and diffing their frame data. Idle-turn animation is not translated -- see the boundary below.
...
- `hrt_torso_anims_table`'s 12 off-diagonal turn-transition entries (`hrt_2_to_4_turn2_anim` etc.) and `set_rotate_anim`/`change_anim1` (the `#zip`/idle-turn case, needing `hrt_rotate_anims_table`'s 12 turn-transition sequences plus `hrt_stand6/8_anim`). Real `FACING_DIR`/`NEW_FACING_DIR` tracking needs a shared "compute `NEW_FACING_DIR`" routine this port hasn't located yet, so both the leg-cycle selector and the torso table substitute `FACING_DIR=NEW_FACING_DIR=MOVE_DIR` while moving -- correct for walking in a straight line, the only case reachable today, but not source-exact once strafing (facing one way while moving another) exists, and it's why the torso table's 12 turn-transition entries can never be reached yet. `CAN_MOVE_DIR` ring-boundary redirection (`execute_walk`'s down-left/down-right special case) is also not translated, since it depends on the unported ring-boundary system.
