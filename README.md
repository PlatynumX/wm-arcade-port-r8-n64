...
- The rest of the already-ported combat core's connection to a live match: `wm_arcade_move_ported_wrestler`'s generic 8-wrestler dispatch isn't used yet (only Bret is wired, directly); the other seven wrestlers have no visual backend or movement at all. `DRONE.ASM`'s AI script/range data (`wnshort_t`/`wnmed_t`/`wnlong_t`, `blkbase_t`/`blkatk_t`/`sklhhdly_t`/`sklhrdly_t`) is emitted by a source macro (`SKLM`) that is not present in the checked-in tree, so it is not guessed -- the drone AI therefore never actually gives a wrestler a button/stick to act on, so `wm/movement.h` has nothing to drive it yet either.
...
- Attract-mode wrestlers hold real health/ring state and Bret can visibly stand in the ring correctly animated and move with real input today, but nothing walks with a real animation, fights, or has a human player behind it yet.
