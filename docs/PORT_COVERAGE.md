...
| `start_match` | **partial-source** | Only the PSTATUS==0 (#0plyr, attract-mode) creation path is translated (wm/match.h), driven from WM_ATTRACT_SHOW_GAMEPLAY -- see the show_gameplay note above for exactly what is and is not real, including the Bret-only visual backend and movement. The #1plyr/#2plyr credited-match paths (WM_APP_MODE_MATCH_INIT after select/pregame) remain an explicit dead-end stub; CLOSE_PROGRESS_SCREEN, ring/rope/crowd/timer process creation, and collision/pin/finisher lifecycle are all still unported. |
