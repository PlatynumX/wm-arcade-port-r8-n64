...
| `show_gameplay` | **partial-source** | WRESTLE.ASM::start_match's PSTATUS==0 (#0plyr) path now runs from the real attract flow: the RNDRNG0(7)-skip-7 index1 draw, LIFEBAR.ASM::init_life_data (LIFE_MAX=163), and #0plyr's PLYRNUM/PSIDE wrestler-actor creation are translated (wm/match.h), and the SLEEP 3*60 + wait_on_butn 10*TSEC timing is exact. Whichever actor draws WM_ROSTER_BRET runs the real wm_arcade_move_bret control layer every tick with a real visual backend (wm/bret_backend.h) and real movement (wm/movement.h): convert_facing, set_velocities, execute_walk's dispatch, and change_walk_anim's leg-cycle reselection (all 12 hrt_walkM_fF_anim sequences) are all translated, so a Bret actor with real input genuinely walks with the correct animation. Its idle stance and 6 light/power punch/kick animations resolve to real wm_visual_sequence data, and each of those 6 now fires ANIM.ASM's real ATTACK_ON/ATTACK_ON_Z/ATTACK_OFF (wm_arcade_ani_attack_on(_z/_off), real since fix38) at the exact frame index hand-traced from each attack's own HRTSEQ2.ASM source, setting real attack_mode/attack_xoff/.../WM_MODE_CHECKHIT. Not translated: wm_arcade_check_wrestler_collisions is still not called from wm_match, and even if it were, victim hurt boxes (wm_arcade_set_hurt_box) need per-frame ANI3 hit-box data (SYS.EQU IANI3X/Y/Z/ID) baked into the WIMP image assets themselves, not the animation script -- a third asset format this port has not parsed -- so no attack can actually land yet. Also not translated: change_walk_anim's TORSO half and set_rotate_anim/change_anim1 (idle-turn animation; real FACING_DIR tracking needs an as-yet-unlocated shared 'compute NEW_FACING_DIR' routine, substituted with MOVE_DIR while moving), INIT_LADDER_TABLE/CURRENT_LADDER/NUM_OPPS multi-drone team selection (one placeholder opponent only), and DRONE.ASM's wnshort_t/wnmed_t/wnlong_t script data and blkbase_t/blkatk_t/sklhhdly_t/sklhrdly_t skill tables (SKLM macro expansion not in the checked-in source tree, so not guessed) -- meaning a CPU-controlled wrestler never actually throws a button/stick input. |
