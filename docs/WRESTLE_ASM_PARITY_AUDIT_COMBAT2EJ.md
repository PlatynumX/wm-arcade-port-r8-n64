# Combat2EJ — WRESTLE.ASM Source-Parity Audit
...
## Combat2EJ hardware-video finding

The supplied ~30 second hardware capture shows the attract/demo wrestlers walking left through the ring boundary while the camera follows them until the ring is empty. This is consistent with the parity gap already identified in `WRESTLE.ASM::wrestler_main`: the arcade loop executes `set_collision_boxes -> confine_wrestler -> confine_wrestler_fix2` before `update_newfacing/drone_main`, and executes `set_collision_boxes -> confine_wrestler -> confine_wrestler_fix1` again after velocity/animation. Combat2EH did neither live confinement call.

Combat2EJ wires a deliberately narrow, source-safe portion of `confine_wrestler` for grounded, unattached, non-zombie in-ring actors while every live opponent is also in-ring. Under those exact preconditions the source's climb-out and airborne rope-bounce branches cannot fire, so the position correction and `CAN_MOVE_DIR` result are a direct translation rather than an approximation. The full outside-ring/attached/airborne confinement path remains a separately tracked parity item.

The new EI Sports-logo replacement is intentionally not included. Combat2EJ starts from the exact Combat2EH baseline sports asset state.
