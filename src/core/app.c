...
static void fix39_bind_demo_frame_boxes(wm_app *app) {
    const wm_visual_frame *vf[2] = { wm_visual_current(&app->demo.p1.visual), wm_visual_current(&app->demo.p2.visual) };
    for (size_t i = 0; i < 2u; ++i) {
        const uint8_t rid = i ? app->demo.p2.roster_id : app->demo.p1.roster_id;
        ...
        if (spr && wm_arcade_wimp_frame_box_from_sprite(spr, &box))
            (void)wm_fix39_match_set_frame_box(i, &box);
        else
...
static bool fix39_tick_gameplay_demo(wm_app *app, const wm_input_state *input) {
    ...
    ++a->call_ticks;
    if (a->call_ticks == 1u) {
        wm_demo_set_roster(&app->demo,
                           (uint8_t)wm_fix39_frontend_to_arcade_roster((unsigned)app->p1_choice),
                           (uint8_t)wm_fix39_frontend_to_arcade_roster((unsigned)app->p2_choice));
        wm_demo_reset_match(&app->demo);
        /* match_begin consumes FRONTEND selection slots and performs the same canonical mapping internally. */
        wm_fix39_match_begin((uint8_t)app->p1_choice, (uint8_t)app->p2_choice);
        wm_fix39_match_set_cpu_vs_cpu(true);
    }

    /* The source-art N64 presenter remains a presentation adapter only. The
       translated arcade DRONE/combat runtime below owns match behavior. */
    {
        wm_input_state cpu1 = {0};
        int dx = app->demo.p2.screen_x - app->demo.p1.screen_x;
        int dy = app->demo.p2.screen_y - app->demo.p1.screen_y;
        if (dx > 36) cpu1.stick_x = 75;
        else if (dx < -36) cpu1.stick_x = -75;
        if (dy > 18) cpu1.stick_y = -55;
        else if (dy < -18) cpu1.stick_y = 55;
        if (dx < 58 && dx > -58 && dy < 30 && dy > -30) {
            unsigned phase = (unsigned)((a->call_ticks / 24u) & 3u);
            cpu1.light_punch = phase == 0u;
            cpu1.power_punch = phase == 1u;
            cpu1.light_kick = phase == 2u;
            cpu1.power_kick = phase == 3u;
        }
        wm_demo_tick(&app->demo, &cpu1);
    }

    /* Source check: WRESTLE.ASM seeds FACING_DIR as the 8-way vector toward
       the opponent (P1=9, P2=6 at reset).  The temporary presenter only has
       horizontal flip state, so derive that bit from the same authoritative
       X positions instead of preserving wm_demo stale back-to-back flip. */
    if (app->demo.p1.screen_x < app->demo.p2.screen_x) {
        app->demo.p1.flip_x = false;
        app->demo.p2.flip_x = true;
    } else if (app->demo.p1.screen_x > app->demo.p2.screen_x) {
        app->demo.p1.flip_x = true;
        app->demo.p2.flip_x = false;
    }

    /* Keep COLLIS in the exact same X/Z/facing space as the source-art presenter. */
    wm_fix39_match_sync_presenter_pose(0, app->demo.p1.screen_x, app->demo.p1.screen_y, app->demo.p1.flip_x, app->demo.p1.action == WM_DEMO_BLOCK);
    wm_fix39_match_sync_presenter_pose(1, app->demo.p2.screen_x, app->demo.p2.screen_y, app->demo.p2.flip_x, app->demo.p2.action == WM_DEMO_BLOCK);
    {
        const wm_visual_frame *af0 = wm_visual_current(&app->demo.p1.visual);
        const wm_visual_frame *af1 = wm_visual_current(&app->demo.p2.visual);
        wm_fix39_match_bind_source_frame_attack(0, app->demo.p1.roster_id, af0 ? af0->source_frame : NULL);
        wm_fix39_match_bind_source_frame_attack(1, app->demo.p2.roster_id, af1 ? af1->source_frame : NULL);
    }

    /* Bind the exact current WIMP IANI3 tuple before the source collision slot. */
    fix39_bind_demo_frame_boxes(app);
    {
        const WmFix39Status *st0 = wm_fix39_status();
        uint32_t hits0 = st0 ? st0->combat_accepted_hits : 0u;
        int life0 = (int)wm_fix39_match_life(0);
        int life1 = (int)wm_fix39_match_life(1);
        wm_fix39_match_tick(0, 0, false, false, false, false, false, false);
        app->demo.p1.health = (int)wm_fix39_match_life(0);
        app->demo.p2.health = (int)wm_fix39_match_life(1);
        /* Presentation follows the real COLLIS/REACT result, never the demo
           harness damage model. This makes accepted source hits visible while
           the final reaction-frame renderer is still being connected. */
        {
            const WmFix39Status *st1 = wm_fix39_status();
            uint32_t hits1 = st1 ? st1->combat_accepted_hits : hits0;
            app->demo.total_hits = hits1;
            if (app->demo.p1.health < life0) {
                app->demo.p1.stun_ticks = 10u;
                app->demo.p2.attack_connected = true;
                ++app->demo.p2.hit_count;
            }
            if (app->demo.p2.health < life1) {
                app->demo.p2.stun_ticks = 10u;
                app->demo.p1.attack_connected = true;
                ++app->demo.p1.hit_count;
            }
            if (hits1 > hits0 && app->demo.p1.health == life0 && app->demo.p2.health == life1)
                ++app->demo.total_blocks;
        }
    }

    if (a->call_ticks > 60u && wm_app_any_attract_button(input)) {
        wm_fix39_match_set_cpu_vs_cpu(false);
        return true;
    }
    if (a->call_ticks >= 452u) {
        wm_fix39_match_set_cpu_vs_cpu(false);
        return true;
    }
    return false;
