                              ...
                              const wm_arcade_actor_t *other)
{
    ...
    if (other->anim_mode & WM_MODE_OVERLAP) return 0;
    ...
    if (mover->anim_mode & WM_MODE_OVERLAP) return 0;
    ...
    const wm_arcade_combat_callbacks_t *callbacks)
{
    ...
    if (victim->anim_mode & WM_MODE_NOCOLLIS) return WM_HIT_REJECTED;
    ...
    if (attacker->anim_mode & WM_MODE_WAITHITOPP) {
        attacker->anim_mode &= (uint16_t)~WM_MODE_WAITHITOPP;
    ...
    attacker->anim_mode |= WM_MODE_STATUS;
    ...
    const wm_arcade_combat_callbacks_t *callbacks)
{
    ...
    for (outer = 0; outer < actor_count; ++outer) {
        ...
        if (!(attacker->anim_mode & WM_MODE_CHECKHIT)) continue;
...
{
    if (actor) actor->anim_mode &= (uint16_t)~WM_MODE_CHECKHIT;
}
