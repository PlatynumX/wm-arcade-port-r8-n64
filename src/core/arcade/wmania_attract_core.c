...
#define ADD(scr, demo, optional) \
    ...
    ADD(WM_ATTRACT_HISCORES, false, false);
    ADD(WM_ATTRACT_DCS_LOGO, false, false);
    ADD(WM_ATTRACT_SPORTS_LOGO, false, false);

    /* Explicit placeholders only: gameplay is intentionally not ported. */
    ADD(WM_ATTRACT_GAMEPLAY_DEMO_1, true, false);

    ADD(WM_ATTRACT_CREDITS_1, false, false);
    ADD(WM_ATTRACT_TITLE, false, false);

    ADD(WM_ATTRACT_GAMEPLAY_DEMO_2, true, false);

    ADD(WM_ATTRACT_CREDITS_2, false, false);
    ADD(WM_ATTRACT_DESIGNER_HINT, false, false);
    ADD(WM_ATTRACT_GENERAL_TIPS, false, false);
    ADD(WM_ATTRACT_BIO, false, false);
    ADD(WM_ATTRACT_BIO_TIPS, false, false);
    ADD(WM_ATTRACT_OPERATOR_MESSAGE, false, true);
    ...
    if ((state->amode_loops & 1u) == 0u) {
        ADD(WM_ATTRACT_EVEN_LOOP_CREDITS, false, false);
        ADD(WM_ATTRACT_TIME_DATE, false, true);
    }

    if ((state->amode_loops & 7u) == 0u) {
        ADD(WM_ATTRACT_COPYRIGHT, false, false);
        ADD(WM_ATTRACT_AAMA, false, false);
