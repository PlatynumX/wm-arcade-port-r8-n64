...
#define WM_ROPE_RUNTIME_GUARD 128u
    ...
    const WmRopeRuntimeAdapter *adapter)
{
    ...
    send_image(adapter, runtime->bank, channel_index,
               WM_ROPE_HALF_FIRST, c->first_image_symbol);

    if (c->second_object_exists) {
        send_image(adapter, runtime->bank, channel_index,
                   WM_ROPE_HALF_SECOND, c->second_image_symbol);
    }
}
    ...
    const WmRopeRuntimeAdapter *adapter)
{
     ...
     */
    while (guard++ < WM_ROPE_RUNTIME_GUARD) {
        const WmRopeFrame *frame;
    ...
    bool reduce_bog)
{
    ...
    for (i = 0u; i < WM_ROPE_CHANNEL_COUNT; ++i) {
        WmRopeRuntimeChannel *c = &runtime->channel[i];

        c->first_object_exists =
            seed->object[i][WM_ROPE_HALF_FIRST].exists;
        c->second_object_exists =
            seed->object[i][WM_ROPE_HALF_SECOND].exists;

        c->first_image_symbol =
            seed->object[i][WM_ROPE_HALF_FIRST].source_image_symbol;
        c->second_image_symbol =
            seed->object[i][WM_ROPE_HALF_SECOND].source_image_symbol;
    ...
    const WmRopeCommandProgram *program)
{
    ...
    for (i = 0u;
         i < program->channel_count && i < WM_ROPE_CHANNEL_COUNT;
         ++i) {
        WmRopeRuntimeChannel *c = &runtime->channel[i];
        ...
        if (first->kind != WM_ROPE_SCRIPT_SEQUENCE ||
