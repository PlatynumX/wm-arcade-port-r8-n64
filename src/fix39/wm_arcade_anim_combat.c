                             ...
                             const wm_arcade_attack_on_args_t *args)
{
    ...
    actor->hit_blocker = 0;
    actor->anim_mode &= (uint16_t)~WM_ARCADE_MODE_STATUS;
    actor->anim_mode |= WM_ARCADE_MODE_CHECKHIT;
                              ...
                              uint16_t round_tickcount)
{
    ...
    actor->anim_mode &= (uint16_t)~(WM_ARCADE_MODE_CHECKHIT | WM_ARCADE_MODE_WAITHITOPP);
...
{
    if (actor)
        actor->anim_mode |= WM_ARCADE_MODE_WAITHITOPP;
}
