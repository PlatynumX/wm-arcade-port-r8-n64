...
{
    if (a->anim_mode & WM_ARCADE_MODE_UNINT) return WM_BRET_STEP_IDLE;
    ...
    if (a->anim_mode & WM_ARCADE_MODE_UNINT) return WM_BRET_STEP_ACTION;
...
static wm_arcade_bret_step_result_t mode_running(wm_arcade_actor_t *a,wm_arcade_actor_t *o,const wm_arcade_bret_env_t *e,const wm_arcade_bret_callbacks_t *cb)
{
    ...
    if (!(a->anim_mode & WM_ARCADE_MODE_UNINT)) {
...
{
    a->x_vel=0;a->z_vel=0;
    if(a->anim_mode&WM_ARCADE_MODE_END){a->move_dir^=(WM_MOVE_LEFT+WM_MOVE_RIGHT);a->facing_dir=(a->new_facing_dir&(WM_MOVE_UP+WM_MOVE_DOWN))|a->move_dir;anim(a,WM_BRET_ANIM_RUN2,cb);setmode(a,WM_PMODE_RUNNING);return WM_BRET_STEP_ACTION;}
    return WM_BRET_STEP_IDLE;
}
...
{
    if(a->anim_mode&WM_ARCADE_MODE_NOGRAVITY){if(cb&&cb->mode_choking)cb->mode_choking(a,cb->user);return WM_BRET_STEP_EXTERNAL;}
...
wm_arcade_bret_step_result_t wm_arcade_move_bret(wm_arcade_actor_t*a,wm_arcade_actor_t*o,const wm_arcade_bret_env_t*e,const wm_arcade_bret_callbacks_t*cb)
{
    ...
    case WM_PMODE_WAITANIM:if((a->anim_mode&WM_ARCADE_MODE_END)&&cb&&cb->code_addr)cb->code_addr(a,a->code_addr,cb->user);return WM_BRET_STEP_EXTERNAL;
...
int wm_arcade_bret_fire_secret(wm_arcade_actor_t*a,wm_arcade_actor_t*o,wm_arcade_bret_secret_id_t id,uint32_t pcnt,const wm_arcade_bret_callbacks_t*cb)
{
    ...
    case WM_BRET_SECRET_SUPERCUT:
        if((a->anim_mode&WM_ARCADE_MODE_UNINT)||!o||o->player_mode==WM_PMODE_ONGROUND||o->player_mode==WM_PMODE_DEAD||o->player_mode==WM_PMODE_HEADHELD||o->player_mode==WM_PMODE_ATTACHED)return 0;
    ...
    case WM_BRET_SECRET_CHARGE_DDT:
        if(a->player_mode==WM_PMODE_HEADHELD||a->player_mode==WM_PMODE_HEADHOLD||(a->anim_mode&WM_ARCADE_MODE_UNINT))return 0;
    ...
    case WM_BRET_SECRET_NECK_GRAB:
        if((a->anim_mode&WM_ARCADE_MODE_UNINT)||!o||o->player_mode==WM_PMODE_ONGROUND||o->player_mode==WM_PMODE_DEAD||o->player_mode==WM_PMODE_HEADHELD)return 0;
    ...
    case WM_BRET_SECRET_GRAB_FLING: case WM_BRET_SECRET_GRAB_FLING2:
        if((a->anim_mode&WM_ARCADE_MODE_UNINT)||!o)return 0;
    ...
    case WM_BRET_SECRET_FACE_RAKE:
        if((a->anim_mode&WM_ARCADE_MODE_UNINT)||a->player_mode==WM_PMODE_ONTURNBKL||!o||o->player_mode==WM_PMODE_ONGROUND||o->player_mode==WM_PMODE_DEAD||o->player_mode==WM_PMODE_HEADHELD)return 0;
                                ...
                                const wm_arcade_bret_callbacks_t *cb)
{
    ...
    case WM_BRET_MON_ROLL_UPPERCUT:
        if ((a->anim_mode & WM_ARCADE_MODE_UNINT) || a->immobilize_time ||
    ...
    case WM_BRET_MON_GRAB_TOSS_AIR:
        if (!o || (a->anim_mode & WM_ARCADE_MODE_UNINT) || a->player_mode == WM_PMODE_HEADHOLD ||
...
{
    if(!a||charge<100||a->getup_time||a->player_mode==WM_PMODE_HEADHELD||a->player_mode==WM_PMODE_HEADHOLD||a->player_mode==WM_PMODE_ONGROUND||a->player_mode==WM_PMODE_DEAD||(a->anim_mode&WM_ARCADE_MODE_UNINT)||!o||o->player_mode==WM_PMODE_ONGROUND||o->player_mode==WM_PMODE_DEAD)return 0;
...
{
    if(!a||charge<100||a->getup_time||a->player_mode==WM_PMODE_HEADHELD||a->player_mode==WM_PMODE_HEADHOLD||a->player_mode==WM_PMODE_ONGROUND||a->player_mode==WM_PMODE_DEAD||(a->anim_mode&WM_ARCADE_MODE_UNINT))return 0;
