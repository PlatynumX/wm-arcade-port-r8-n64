...
static wm_arcade_doink_step_result_t mode_normal(wm_arcade_actor_t*a,wm_arcade_actor_t*o,const wm_arcade_doink_env_t*e,const wm_arcade_doink_callbacks_t*c){
    uint8_t ac;
    if(a->anim_mode&WM_ARCADE_MODE_UNINT)return WM_DOINK_STEP_IDLE;
    ...
    if(o&&o->player_mode==WM_PMODE_DEAD&&!(o->status_flags&WM_STATUS_ZOMBIE)){
        ...
        if(!reciprocal){
            if((c&&c->teammate_pin&&c->teammate_pin(a,c->user))||(c&&c->raisearm_check&&c->raisearm_check(a,c->user))){anim(a,face_label(L.raise2,L.raise4,a),c);if(c->set_raisearm_bit)c->set_raisearm_bit(a,c->user);return WM_DOINK_STEP_ACTION;}
    ...
    if(a->anim_mode&WM_ARCADE_MODE_UNINT)return WM_DOINK_STEP_ACTION;
...
static wm_arcade_doink_step_result_t mode_bouncing(wm_arcade_actor_t*a,const wm_arcade_doink_callbacks_t*c){a->x_vel=0;a->z_vel=0;if(a->anim_mode&WM_ARCADE_MODE_END){a->move_dir^=(WM_MOVE_LEFT+WM_MOVE_RIGHT);a->facing_dir=(a->new_facing_dir&(WM_MOVE_UP+WM_MOVE_DOWN))|a->move_dir;anim(a,L.run,c);setmode(a,WM_PMODE_RUNNING);return WM_DOINK_STEP_ACTION;}return WM_DOINK_STEP_IDLE;}
...
static wm_arcade_doink_step_result_t mode_headhold(wm_arcade_actor_t*a,wm_arcade_actor_t*o,const wm_arcade_doink_callbacks_t*c){
    uint8_t ac;if(!o||o->player_mode!=WM_PMODE_HEADHELD){a->z_fixed-=6<<16;setmode(a,WM_PMODE_NORMAL);return WM_DOINK_STEP_ACTION;}if(a->anim_mode&WM_ARCADE_MODE_UNINT)return WM_DOINK_STEP_IDLE;ac=action_table[a->but_val_down&WM_BTN_ATTACK_MASK];
...
int wm_arcade_doink_fire_secret(wm_arcade_actor_t*a,wm_arcade_actor_t*o,wm_arcade_doink_secret_id_t id,uint32_t pcnt,const wm_arcade_doink_callbacks_t*c){
    ...
    case WM_DOINK_SECRET_HAMMER: if((a->anim_mode&WM_ARCADE_MODE_UNINT)||a->player_mode==WM_PMODE_ONTURNBKL)return 0; anim(a,"dnk_4_hammer_anim",c); return 1;
