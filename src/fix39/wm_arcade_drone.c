...
void wm_arcade_drone_commit_inputs(wm_arcade_actor_t *a,
    ...
    if (!a || !d) return;
    if (d->force_old_but_zero) {
        old_but = 0u;
        d->force_old_but_zero = 0u;
    }
    ...
    while (d->script_pc < script->op_count && guard++ < script->op_count + 32u) {
        ...
        switch (op->opcode) {
            ...
            case WM_DRONE_SC_CALL_FUNCTION: {
                ...
                size_t before_pc;
                int ran;
                /* Combat2EG executes the 15 source bodies directly.  A handler
                 * may yield (return 0), end the source script, or redirect A9
                 * to another decoded script. */
                if (!cb || !cb->script_call) return WM_DRONE_STEP_SCRIPT;
                before_script = d->script;
                before_pc = d->script_pc;
                ran = cb->script_call(self, opp, d, op->source_label, cb->user);
                if (!ran) return WM_DRONE_STEP_SCRIPT;

                if (!d->script)
                    return (d->but != 0u || d->joy != 0u)
                        ? WM_DRONE_STEP_INPUT : WM_DRONE_STEP_ABORT_SCRIPT;

                if (d->script != before_script || d->script_pc != before_pc)
                    return WM_DRONE_STEP_SCRIPT;
...
wm_arcade_drone_step_result_t wm_arcade_drone_main(
    ...
    d->source_alive_team = alive_team;
    if (rank != 0) {
        if (!d->script) {
            int oldmode = d->mode;
            d->mode = -3;
    ...
    /* A source DRN_BUTCHRG_p continuation is an active script and resumes
       before selecting a new action once its current delay has expired. */
    if (d->charge_kind != 0 && d->charge_pc != 0) {
        result = wm_arcade_drone_native_charge_step(self, opp, d, cb);
        goto done;
    }
