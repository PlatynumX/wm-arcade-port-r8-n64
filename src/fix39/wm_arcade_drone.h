...
typedef struct wm_arcade_drone_state {
    ...
    /*
     * Combat2EG: execution state for the 15 source DRONE executable seams.
     * These fields are the portable equivalent of resuming through A9/EXGPC
     * after DS_CODEEND/DS_SLP1; they are not new gameplay state.
     */
    int32_t source_alive_team; /* source B3 cached by drone_main */
    uint16_t source_old_joy;   /* source A7 cached at routine entry */
    uint16_t force_old_but_zero; /* drn_ontb #jmp clears source A6 */
    int32_t service_pc;
    int32_t service_aux0;
    int32_t service_aux1;
    const char *anim_request;  /* source change_anim1a request (drn_taunt) */

    /*
     * drone_chrg stores a script continuation in DRN_BUTCHRG_p.  The source
     * continuations are represented explicitly here instead of inventing
     * synthetic DRONE script labels.
     */
    int32_t charge_kind;
    int32_t charge_pc;
    int32_t charge_aux;
...
typedef struct wm_arcade_drone_callbacks {
    ...
    /* Return nonzero only when the exact source target executed.  C4d is
       fail-closed: an unresolved command-5/EXGPC seam must not be skipped. */
    int (*script_call)(wm_arcade_actor_t *self,
