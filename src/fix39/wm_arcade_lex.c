...
static wm_arcade_lex_step_result_t mode_normal(wm_arcade_actor_t*a,wm_arcade_actor_t*o,const wm_arcade_lex_env_t*e,const wm_arcade_lex_callbacks_t*c){
    uint8_t ac;
    if(a->anim_mode&WM_ARCADE_MODE_UNINT)return WM_LEX_STEP_IDLE;
    ...
    if(a->anim_mode&WM_ARCADE_MODE_UNINT)return WM_LEX_STEP_ACTION;
...
static wm_arcade_lex_step_result_t mode_bouncing(wm_arcade_actor_t*a,const wm_arcade_lex_callbacks_t*c){a->x_vel=0;a->z_vel=0;if(a->anim_mode&WM_ARCADE_MODE_END){a->move_dir^=(WM_MOVE_LEFT+WM_MOVE_RIGHT);a->facing_dir=(a->new_facing_dir&(WM_MOVE_UP+WM_MOVE_DOWN))|a->move_dir;anim(a,L.run,c);setmode(a,WM_PMODE_RUNNING);return WM_LEX_STEP_ACTION;}return WM_LEX_STEP_IDLE;}
...
static wm_arcade_lex_step_result_t mode_headhold(wm_arcade_actor_t*a,wm_arcade_actor_t*o,const wm_arcade_lex_callbacks_t*c){
    uint8_t ac;if(!o||o->player_mode!=WM_PMODE_HEADHELD){a->z_fixed-=6<<16;setmode(a,WM_PMODE_NORMAL);return WM_LEX_STEP_ACTION;}if(a->anim_mode&WM_ARCADE_MODE_UNINT)return WM_LEX_STEP_IDLE;ac=action_table[a->but_val_down&WM_BTN_ATTACK_MASK];
...
int wm_arcade_lex_fire_secret(wm_arcade_actor_t*a,wm_arcade_actor_t*o,wm_arcade_lex_secret_id_t id,uint32_t pcnt,const wm_arcade_lex_callbacks_t*c){
    ...
    case WM_LEX_SECRET_HAMMER: if((a->anim_mode&WM_ARCADE_MODE_UNINT)||a->player_mode==WM_PMODE_OPPOVERHEAD)return 0; startsp(a,"lex_hammer",c); return 1;
    default:return 0;}
}
...
void wm_arcade_lex_ani_init(wm_arcade_actor_t *a, const wm_arcade_lex_callbacks_t *cb)
{
    const int face2 = a && ((a->facing_dir & WM_MOVE_RIGHT) != 0u);
    if (!a || !cb) return;

    /* Character LEX.ASM ANI_INIT animation body.  WRESTLE.ASM's
       common ani_init clears ATTIMG_CUR_FRAME before dispatch. */
    a->attimg_cur_frame = 0;
    if (cb->change_anim_label)
        cb->change_anim_label(a, face2 ? "lex_stand2_anim" : "lex_stand4_anim", cb->user);
    if (cb->change_torso_label)
        cb->change_torso_label(a, face2 ? "lex_torso2_anim" : "lex_torso4_anim", cb->user);
}
