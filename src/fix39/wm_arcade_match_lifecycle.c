...
void wm_arcade_match_lifecycle_init(wm_arcade_match_lifecycle *m,
                                    bool attract_wrap)
{
    if (!m)
        return;
    ...
    m->ones = 9u;
    m->step = 1500u;
    m->startup_delay = (uint16_t)(2u * WM_ARCADE_TSEC);
    m->pin_timeout = (uint16_t)(4u * WM_ARCADE_TSEC);
    m->attract_wrap = attract_wrap;
}
...
void wm_arcade_match_lifecycle_tick(wm_arcade_match_lifecycle *m,
