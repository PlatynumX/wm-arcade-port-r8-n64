...
{
 if(a->anim_mode&WM_ARCADE_MODE_UNINT)return WM_RZR_STEP_IDLE;
 ...
 if(a->anim_mode&WM_ARCADE_MODE_UNINT)return WM_RZR_STEP_ACTION;
...
static wm_arcade_razor_step_result_t mode_running(wm_arcade_actor_t*a,wm_arcade_actor_t*o,const wm_arcade_razor_env_t*e,const wm_arcade_razor_callbacks_t*cb)
{
 ...
 if(!(a->anim_mode&WM_ARCADE_MODE_UNINT)&&(((a->stick_val_cur|a->move_dir)&(WM_MOVE_LEFT|WM_MOVE_RIGHT))==(WM_MOVE_LEFT|WM_MOVE_RIGHT)))setmode(a,WM_PMODE_NORMAL);
...
{a->x_vel=0;a->z_vel=0;if(a->anim_mode&WM_ARCADE_MODE_END){a->move_dir^=(WM_MOVE_LEFT+WM_MOVE_RIGHT);a->facing_dir=(a->new_facing_dir&(WM_MOVE_UP+WM_MOVE_DOWN))|a->move_dir;anim(a,WM_RZR_ANIM_RUN2,cb);setmode(a,WM_PMODE_RUNNING);return WM_RZR_STEP_ACTION;}return WM_RZR_STEP_IDLE;}
...
{if(!a||charge<85||a->player_mode==WM_PMODE_HEADHELD||a->player_mode==WM_PMODE_HEADHOLD||a->getup_time||(a->anim_mode&WM_ARCADE_MODE_UNINT)||!o||groundish(o))return 0;if(cb&&cb->ck_ignore&&cb->ck_ignore(a,cb->user))return 0;setmode(a,WM_PMODE_INAIR);a->x_vel=1;anim(a,WM_RZR_ANIM_FLYING_KICK,cb);snd(a,WM_RZR_SND_FLYKICK,cb);return 1;}
int wm_arcade_razor_release_charge_slashes(wm_arcade_actor_t*a,uint16_t charge,const wm_arcade_razor_callbacks_t*cb)
{if(!a||charge<100||(a->anim_mode&WM_ARCADE_MODE_UNINT)||a->getup_time||a->player_mode==WM_PMODE_HEADHELD||a->player_mode==WM_PMODE_HEADHOLD||a->player_mode==WM_PMODE_ONGROUND||a->player_mode==WM_PMODE_DEAD)return 0;a->special_move_addr=(uintptr_t)WM_RZR_ANIM_REPEAT_SLASH;snd(a,WM_RZR_SND_KICK_T2,cb);return 1;}
