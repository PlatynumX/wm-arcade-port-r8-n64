                         ...
                         bool block)
{
    ...
    /* ANIM/COLLIS source ownership: current animation frame now drives both
    ...
    /* wrestler_main per-tick countdown tail.  These fields are decremented
       exactly once when nonzero, after update_joy_dtime in the source loop. */
    for (i = 0u; i < WM_FIX39_ACTOR_COUNT; ++i) {
        wm_arcade_actor_t *a = &g.actors[i];
        ...
        /* Combat2BY GETUP_TIME: source-owned knockdown recovery countdown. */
        if (a->getup_time > 0) {
            --a->getup_time;
            if (a->getup_time == 0 && (a->anim_mode & WM_ARCADE_MODE_END)) {
                const char *stand = live_default_stand_label(a);
                a->anim_mode &= (uint16_t)~(WM_ARCADE_MODE_UNINT | WM_ARCADE_MODE_END);
                if (a->player_mode == WM_PMODE_ONGROUND) a->player_mode = WM_PMODE_NORMAL;
                if (stand) (void)live_start_source_anim(a, stand, false);
            }
        }
    }
