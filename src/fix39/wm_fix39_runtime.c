...
static int live_has_live_teammates(const wm_arcade_actor_t *victim, void *user)
{
    ...
    /* WRESTLE2.ASM ck_live_teammates: walk process_ptrs, skip inactive
       slots and self, require matching PLYR_SIDE, then reject MODE_DEAD.
       The arcade routine returns carry set on the first live teammate. */
    for (i = 0u; i < WM_FIX39_ACTOR_COUNT; ++i) {
