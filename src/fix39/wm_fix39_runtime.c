...
static void init_drone_callbacks(void)
{
    ...
    g.status.drone_plain_rnd_ready = false;
    g.status.drone_scripts_ready = wm_arcade_drone_source_scripts_ready();
    /* C4 still owns plain rnd, drone_seek and source code-call services. */
...
void wm_fix39_match_begin(unsigned frontend_p1, unsigned frontend_p2)
{
    ...
    /* V13e-c3 has bound fetched DRONE.ASM scalar/range tables, RNDRNG0,
       decoded script bodies and command-2 skill tables.  The distinct plain
       `rnd`, drone_seek and source code-call services remain C4 gates. */
    g.status.drone_scalar_tables_ready = wm_arcade_drone_source_tables_ready();
