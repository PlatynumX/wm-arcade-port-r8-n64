...
static void init_drone_callbacks(void)
{
    ...
    g.status.drone_scripts_ready = false;
...
void wm_fix39_match_begin(unsigned frontend_p1, unsigned frontend_p2)
{
    ...
    /* V13e-c1 has now bound the fetched DRONE.ASM scalar tables and exact
       RNDRNG0 service.  The distinct plain `rnd` service plus the decoded
       Stage25 range/script payload remain the gates before drone_main is live. */
    g.status.drone_scalar_tables_ready = wm_arcade_drone_source_tables_ready();
