...
void wm_fix39_match_bind_bret_demo_attack(size_t index, int action,
                                          unsigned frame_index, int facing)
{
    wm_arcade_actor_t *a;
    bool active = false;
    if (index >= WM_FIX39_ACTOR_COUNT) return;
    ...
    /* wm_demo_action: LIGHT_PUNCH=4, POWER_PUNCH=5.  The presenter frame
       indices below are the zero-based source-art indices already used by
       demo.c (FR4/FR6 respectively). */
    if (action == 4 && frame_index == 5u) {
        wm_arcade_attack_on_z_args_t args;
        args.attack_mode = WM_AMODE_PUNCH;
        args.xoff = 30;
        args.yoff = 91;
        args.zoff = (facing == 2) ? -45 : 0;
        args.width = 50;
        args.height = 15;
        args.depth = 45;
        if (!(a->anim_mode & WM_ARCADE_MODE_CHECKHIT))
            wm_arcade_ani_attack_on_z(a, &args);
        active = true;
    } else if (action == 5 && frame_index == 5u) {
        wm_arcade_attack_on_args_t args;
        args.attack_mode = WM_AMODE_UPRCUT;
        args.xoff = -6;
        args.yoff = 40;
        args.width = 64;
        args.height = 90;
    ...
    if (!active && (a->anim_mode & WM_ARCADE_MODE_CHECKHIT))
