...
{
    if(!a)return 0; return a->who_i_hit?a->who_i_hit:(a->attach_proc?a->attach_proc:a->smart_target);
}
static void source_native_pause_opp(wm_arcade_actor_t*a){wm_arcade_actor_t*o=source_native_opp(a);if(!o)return;o->ani_count=25;o->anim_mode|=WM_ARCADE_MODE_UNINT;}
...
static void source_native_clear_opp_counts(wm_arcade_actor_t*a){wm_arcade_actor_t*o=a?a->attach_proc:0;if(!o)return;o->punchb_count=o->blockb_count=o->spunchb_count=o->kickb_count=o->skickb_count=0;}
static void source_native_head_grab_time(wm_arcade_actor_t*a){if(!a)return;a->last_headhold=g.status.pcnt;source_native_clear_opp_counts(a);}
static void source_native_check_xvel(wm_arcade_actor_t*a){if(!a)return;if(a->facing_dir&WM_MOVE_RIGHT){if(a->x_vel<=0)a->x_vel=0x20000;}else if(a->x_vel>=0)a->x_vel=-0x20000;a->z_vel=0;}
...
static void source_native_reattach(wm_arcade_actor_t*a){wm_arcade_actor_t*o=a?a->who_i_hit:0;if(!a||!o)return;a->attach_proc=o;o->attach_proc=a;}
static void source_native_ck_dead_opp(wm_arcade_actor_t*a){wm_arcade_actor_t*o=source_native_opp(a);if(!a)return;a->anim_mode&=(uint16_t)~WM_ARCADE_MODE_STATUS;if(o&&o->life<=0)a->anim_mode|=WM_ARCADE_MODE_STATUS;}
...
static void source_native_halve_bk_xvel(wm_arcade_actor_t*a){int32_t x;if(!a)return;x=a->x_vel;if(!(a->facing_dir&WM_MOVE_RIGHT))x=-x;if(x<0)a->x_vel>>=1;}
static void source_native_fix_bnc_flip(wm_arcade_actor_t*a){if(!a)return;if(a->x_int<WM_RING_X_CENTER)a->obj_control|=WM_OBJ_FLIPH;else a->obj_control&=(uint16_t)~WM_OBJ_FLIPH;}
static void source_native_set_dir_face(wm_arcade_actor_t*a){if(!a)return;if(a->in_ring)a->new_facing_dir=(a->x_int<WM_RING_X_CENTER)?10:6;else a->new_facing_dir=(a->x_int<WM_RING_X_CENTER)?6:10;}
static void source_native_set_trgt(wm_arcade_actor_t*a){if(!a)return;a->tgt_xoff=(a->x_int<WM_RING_X_CENTER)?(WM_RING_X_CENTER-0xf8-60):(WM_RING_X_CENTER+0xf8+60);a->tgt_zoff=WM_RING_Z_CENTER;a->tgt_yoff=WM_MAT_Y;}
...
static void source_native_set_zvel1(wm_arcade_actor_t*a){if(!a)return;if(a->facing_dir&WM_MOVE_UP)a->anim_mode&=(uint16_t)~WM_ARCADE_MODE_STATUS;else a->anim_mode|=WM_ARCADE_MODE_STATUS;}
...
static void source_native_check_raisearm(wm_arcade_actor_t*a){if(!a)return;if(a->status_flags&WM_STATUS_DID_RAISEARM)a->anim_mode&=(uint16_t)~WM_ARCADE_MODE_STATUS;else a->anim_mode|=WM_ARCADE_MODE_STATUS;}
static void source_native_buckoff_vels(wm_arcade_actor_t*a){if(!a)return;a->x_vel=(a->x_int<=WM_RING_X_CENTER)?0x20000:-0x20000;a->z_vel=(a->z_int<=WM_RING_Z_CENTER)?0x40000:-0x40000;a->y_vel=0x50000;}
...
static void source_native_free_toss_check(wm_arcade_actor_t*a){wm_arcade_actor_t*o=a?a->smart_target:0;if(!a)return;a->anim_mode|=WM_ARCADE_MODE_STATUS;if(o&&abs32(o->z_int-a->z_int)>=15)a->anim_mode&=(uint16_t)~WM_ARCADE_MODE_STATUS;}
static void source_native_setup_freetoss(wm_arcade_actor_t*a){wm_arcade_actor_t*o=a?a->who_i_hit:0;if(!a)return;a->anim_mode=0;if(o){o->immobilize_time=20;a->smart_target=o;}}
static void source_native_clr_climb(wm_arcade_actor_t*a){if(a){a->climbing_thru=0;a->safe_time=1;}}
static void source_native_set_opp_y(wm_arcade_actor_t*a){wm_arcade_actor_t*o=a?a->who_i_hit:0;int32_t x;if(!o)return;o->y_vel=0x50000;o->z_vel=0x20000;x=-0x30000;if(!(o->new_facing_dir&WM_MOVE_RIGHT))x=-x;o->x_vel=x;}
static void source_native_set_wrestler_xflip(wm_arcade_actor_t*a){if(!a)return;if(a->facing_dir&WM_MOVE_RIGHT)a->obj_control&=(uint16_t)~WM_OBJ_FLIPH;else a->obj_control|=WM_OBJ_FLIPH;}
static void source_native_hit_ground(wm_arcade_actor_t*a){if(!a)return;a->y_int=a->ground_y;a->y_fixed=a->ground_y<<16;}
...
static void source_native_ckongrnd(wm_arcade_actor_t*a){wm_arcade_actor_t*o=a?a->smart_target:0;if(!a)return;a->anim_mode&=(uint16_t)~WM_ARCADE_MODE_STATUS;if(o&&o->player_mode==WM_PMODE_ONGROUND)a->anim_mode|=WM_ARCADE_MODE_STATUS;}
static void source_native_get_off(wm_arcade_actor_t*a,int which){if(!a)return;if(which==4){a->z_vel=-0x20000;a->y_vel=0x10000;}else{a->z_vel=0x30000;a->y_vel=0x20000;}}
...
static void source_native_target_whoihit(wm_arcade_actor_t*a){if(!a)return;a->status_flags|=WM_STATUS_SMART_ATTACK;a->smart_target=a->who_i_hit;}
static void source_native_blocked_vels(wm_arcade_actor_t*a){if(a){a->y_vel=0x30000;a->x_vel=-(a->x_vel>>1);}}
static void source_native_optimal_position(wm_arcade_actor_t*a){wm_arcade_actor_t*o=a?a->who_i_hit:0;int32_t dx;if(!a||!o)return;dx=0x460000;if(a->facing_dir&WM_MOVE_LEFT)dx=-dx;o->x_fixed=a->x_fixed+dx;o->x_int=o->x_fixed>>16;}
...
static void source_native_pause_state(wm_arcade_actor_t*a){if(a)a->anim_mode|=WM_ARCADE_MODE_UNINT;}
...
/* Combat2BN: direct WRESTLE.ASM::execute_walk/set_velocities translation.
   All eight shipped wrestlers use the same final #VEL/#DVEL constants in the
   historical source: #VEL=0x3a000, #DVEL=0x31000.  MOVE_DIR is converted by
   WRESTLE.ASM::convert_facing into this 8-way X/Z velocity table. */
static void source_execute_walk(wm_arcade_actor_t *a, void *user)
{
    ...
    /* execute_walk clears ATTACK_TYPE before selecting the walk routine. */
    a->attack_type = 0;
    ...
    /* The individual walk-table entries set horizontal flip directly. */
    ...
    /* WRESTLE.ASM MULT=90% while backing away, except when WALK_FAST is set
       or the opponent is down/dead.  Against a down/dead opponent X receives
       GRND_MULT=150%; Z does not take that ground multiplier in source. */
    if (!a->walk_fast && opp &&
        opp->player_mode != WM_PMODE_ONGROUND && opp->player_mode != WM_PMODE_DEAD) {
                                        ...
                                        bool flip_x, bool blocking)
{
    ...
    /* Presentation-space X/Z and flip are retained only for diagnostics.
       WRESTLE/WRESTLE2 translated movement owns actor world coordinates and
       facing; never feed presenter screen coordinates back into source state. */
    (void)x;
    (void)z;
    (void)flip_x;
