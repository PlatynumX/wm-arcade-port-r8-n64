                         ...
                         bool block)
{
    ...
    /* Temporary presenter contributes source frame attack metadata only.
       It no longer writes X/Z into the translated wrestler state.  WRESTLE
       movement owns world position, as required by scroll_world. */
    for (i = 0u; i < WM_FIX39_ACTOR_COUNT; ++i) {
                                             ...
                                             const char *source_frame)
{
    ...
    active = wm_arcade_character_attack_for_source_frame(roster_id, source_frame,
                                                          &uses_z, &zargs, &args);
