...
typedef struct {
    ...
    /* V13e chunked DRONE binding state: scalar source data and RNDRNG0 are
       separated from the still-unbound plain RND + script payloads. */
...
/* Source-backed DRONE callback bundle under incremental binding.  V13e-c2
 * supplies RNDRNG0, exact scalar tables, and exact wnshort/wnmed/wnlong mode
 * + script-list metadata.  Plain RND and decoded script bodies remain later
 * chunks. */
