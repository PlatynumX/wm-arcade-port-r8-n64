    ...
    const WmAttractAdapter *adapter)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        case WM_FIX39_ATTRACT_GAMEPLAY_DEMO_2:
            if (adapter->start_gameplay_demo != 0) {
                WmAttractDemoPlan plan;
                wm_attract_demo_plan_make(&plan, step->source_amode_loops,
                    music_adjustment(adapter), adapter->demo_rng,
                    adapter->demo_rng_user);
                adapter->start_gameplay_demo(
                    adapter->user,
