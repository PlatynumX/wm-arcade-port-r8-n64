...
void wm_attract_demo_plan_make(WmAttractDemoPlan *plan,
                               ...
                               void *rng_user)
{
    /* ATTR.ASM::show_gameplay source order is significant:
       RNDRNG0(7) player -> INIT_LADDER_TABLE (which consumes RNG) ->
       RNDRNG0(5)+1 battle.  Earlier ports sampled the battle before building
       the ladder, changing both opponent identity and subsequent RAND state. */
    uint8_t temp[8] = {7,6,5,4,3,2,1,0};
    uint8_t ladder[7][3] = {{0}};
    static const uint8_t counts[7] = {1,1,1,1,2,2,3};
    uint32_t wrestler;
    uint32_t battle;
    unsigned temp_pos = 8u;
    unsigned e, j;
    if (!plan) return;
    memset(plan, 0, sizeof(*plan));
    wrestler = rng ? rng(rng_user, 7u) : 0u;
    if (wrestler == 7u) ++wrestler;

    /* PROGRESS.ASM::RANDOMIZE_ORDER / FETCH_NEXT_OPPONENT. */
    for (e = 0u; e < 7u; ++e) {
        for (j = 0u; j < counts[e]; ++j) {
            if (temp_pos >= 8u) {
                unsigned pos;
                for (pos = 0u; pos < 8u; ++pos) {
                    uint32_t max = 7u - pos;
                    uint32_t pick = rng ? rng(rng_user, max) : 0u;
                    uint8_t v = temp[pos];
                    temp[pos] = temp[pos + pick];
                    temp[pos + pick] = v;
                }
                temp_pos = 0u;
            }
            ladder[e][j] = temp[temp_pos++];
        }
    }
    battle = (rng ? rng(rng_user, 5u) : 0u) + 1u;
