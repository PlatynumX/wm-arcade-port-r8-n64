...
typedef enum {
    ...
    /* ATTRACT.ASM::show_hstd. The first two pages scroll three rows at a
       time; the remaining three are fixed five-second tables. */
    WM_ATTRACT_LIVE_HSTD_TRANSITION,
    WM_ATTRACT_LIVE_HSTD_SCROLL,
    WM_ATTRACT_LIVE_HSTD_GROUP_HOLD,
    WM_ATTRACT_LIVE_HSTD_FINAL_HOLD,
    WM_ATTRACT_LIVE_HSTD_STATIC_HOLD,
...
typedef struct {
    ...
    uint8_t hiscore_start_rank;
    uint8_t hiscore_rows_since_hold;
    uint8_t hiscore_scroll_px;
    uint8_t hiscore_scroll_duration;
} WmAttractLive;
