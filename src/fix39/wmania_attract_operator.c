    ...
    size_t capacity)
{
    ...
    if (!message || !message->bytes || !out || capacity == 0u ||
        line_index >= message->line_count || message->line_size == 0u) {
        return 0u;
    }

    src = message->bytes + line_index * (size_t)message->line_size;
    while (n < message->line_size && src[n] != 0u) {
        if (n + 1u >= capacity) {
            out[capacity - 1u] = '\0';
            return capacity - 1u;
        }
