...
/* Copy one CMOS row exactly as show_operatormsg reads it: bytes from the
 * row start through the first NUL, bounded by CMESS_LINE_SIZE. Returns the
 * copied character count, excluding the terminator. */
size_t wm_attract_operator_copy_line(
    const WmAttractOperatorMessage *message,
    size_t line_index,
    char *out,
    size_t capacity);
