...
{
    const int32_t t1 =
        (int32_t)(WM_RING_TOP + 1 * WM_FIX39_ROPE_LANE_WIDTH) << 16;
    const int32_t t2 =
        (int32_t)(WM_RING_TOP + 2 * WM_FIX39_ROPE_LANE_WIDTH) << 16;
    const int32_t t3 =
        (int32_t)(WM_RING_TOP + 3 * WM_FIX39_ROPE_LANE_WIDTH) << 16;
    const int32_t t4 =
        (int32_t)(WM_RING_TOP + 4 * WM_FIX39_ROPE_LANE_WIDTH) << 16;
...
{
    if (action == WM_FIX39_ROPE_Z_HIGH) {
        return WM_FIX39_ROPE_HIGH_SECOND_HALF_Z;
    }
    ...
    WmRopeCommand *out)
{
    ...
    if (out == 0 ||
        (unsigned)bank > WM_FIX39_ROPE_RIGHT ||
        (unsigned)action >= WM_FIX39_ROPE_COMMAND_COUNT) {
        return false;
    }
     ...
     */
    if (bank == WM_FIX39_ROPE_FRONT || bank == WM_FIX39_ROPE_BACK) {
        if (action != WM_FIX39_ROPE_BOUNCE_UD || selector >= 4u) {
            return false;
        }

        label = bank == WM_FIX39_ROPE_FRONT ?
            front_ud[selector] : back_ud[selector];
        priority = WM_FIX39_ROPE_SHAKE_PRIORITY;
    } else {
        switch (action) {
        case WM_FIX39_ROPE_BOUNCE_UD:
            if (selector >= 4u) return false;
            label = side_ud[selector];
            priority = WM_FIX39_ROPE_SHAKE_PRIORITY;
            break;

        case WM_FIX39_ROPE_BOUNCE_IO:
            label = "side_bounceio_t";
            priority = WM_FIX39_ROPE_SHAKE_PRIORITY;
            break;

        case WM_FIX39_ROPE_SIDE_SPRING:
            if (selector >= 6u) return false;
            lane = wm_rope_side_lane_from_z_fp16(wrestler_z_fp16);
            label = sspring[lane][selector];
            if (label == 0) return false;
            priority = WM_FIX39_ROPE_SIDE_SPRING_PRIORITY;
            break;

        case WM_FIX39_ROPE_DOWN_SPRING:
            if (selector >= 6u) return false;
            lane = wm_rope_side_lane_from_z_fp16(wrestler_z_fp16);
            label = dspring[lane][selector];
            priority = WM_FIX39_ROPE_DOWN_SPRING_PRIORITY;
            break;

        case WM_FIX39_ROPE_SIDE_SPRING_RELEASE:
            label = "sspr_trans_t";
            priority = WM_FIX39_ROPE_SIDE_SPRING_PRIORITY;
            break;

        case WM_FIX39_ROPE_DOWN_SPRING_RELEASE:
            label = "dspr_trans_t";
            priority = WM_FIX39_ROPE_DOWN_SPRING_PRIORITY;
            break;
