...
const WmRopeBankSeed wm_rope_bank_seeds[4] = {
    {
        WM_FIX39_ROPE_FRONT, true, false,
        {
            {
    ...
    {
        WM_FIX39_ROPE_RIGHT, false, true,
        {
            {
...
{
    if ((unsigned)bank > WM_FIX39_ROPE_RIGHT) {
        return 0;
    }
    ...
    WmRopeHalf half)
{
    ...
    if (b == 0 ||
        (unsigned)channel >= WM_FIX39_ROPE_CHANNEL_COUNT ||
        (unsigned)half > WM_FIX39_ROPE_HALF_SECOND) {
        return 0;
    }
    ...
    bool reduce_bog)
{
    ...
    return bank == WM_FIX39_ROPE_LEFT || bank == WM_FIX39_ROPE_RIGHT;
}
