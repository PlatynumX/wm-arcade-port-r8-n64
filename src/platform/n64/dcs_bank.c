...
static wav64_t wave[LAST + 1];
static bool opened[LAST + 1];

static const entry *find(uint16_t c)
{
    size_t i;

    for (i = 0; i < sizeof(bank) / sizeof(bank[0]); ++i) {
        if (bank[i].command == c)
            return &bank[i];
    }
...
static bool start(const entry *e, int ch, uint16_t raw, uint32_t tick)
{
    if (!e || ch < FIRST || ch > LAST)
        return false;

    closech(ch);
    wav64_open(&wave[ch], e->path);
    wav64_set_loop(&wave[ch], false);
    opened[ch] = true;
    wav64_play(&wave[ch], ch);
    mixer_ch_set_vol(ch, 1.0f, 1.0f);
    debugf("audio: DCS %u asset %u ch%d @%lu\\n",
           (unsigned)raw, (unsigned)e->command, ch, (unsigned long)tick);
    return true;
}
...
bool wm_dcs_bank_play_source(uint16_t c, uint32_t tick, int8_t sc)
{
    const entry *e = find(c);

    if (!e)
        e = find((uint16_t)(c & ~3u));

    if (!e)
        return false;

    return start(e, (int)sc + 1, c, tick);
}

bool wm_dcs_bank_play(uint16_t c, uint32_t tick)
{
    const entry *e = find(c);

    return e ? start(e, e->fallback_channel, c, tick) : false;
}

void wm_dcs_bank_stop_source(int8_t sc)
{
    if (sc >= 0 && sc < 4)
        closech((int)sc + 1);
}

void wm_dcs_bank_set_source_volume(int8_t sc, uint8_t v)
{
    if (sc >= 0 && sc < 4) {
        float f = (float)v / 255.0f;
        mixer_ch_set_vol((int)sc + 1, f, f);
    }
}

void wm_dcs_bank_set_master_volume(uint8_t v)
{
    mixer_set_vol((float)v / 255.0f);
}
