...
static void fix39_draw_rope_object(WmRopeBank bank,WmRopeChannel ch,WmRopeHalf half) {
    ...
    /* BEGINOBJ world coordinates followed by DISPLAY.ASM WORLDTL subtraction. */
    const float sx=(float)(((int)s->raw_x+104)-(WM_RING_X_CENTER-200))*WM_FRONTEND_SCALE_X;
    const float sy=(float)(((int)s->raw_y-258)-(-27))*WM_FRONTEND_SCALE_Y;
    ...
    for(int top=0;top<a->height;top+=sh){int h=a->height-top;if(h>sh)h=sh;
        rdpq_tex_blit(&tex,sx,sy,&(rdpq_blitparms_t){.t0=top,.height=h,.cx=a->xani,.cy=a->yani-top,.flip_x=s->flip_horizontal,.scale_x=WM_FRONTEND_SCALE_X,.scale_y=WM_FRONTEND_SCALE_Y,.filtering=false});}
}
...
static const uint8_t *fix39_arena_image_load(uint16_t header_index) {
    ...
    const wm_ring_arena_image *a=wm_ring_arena_image_at(header_index); if(!a)return NULL;
    const size_t bytes=(size_t)a->width*(size_t)a->height;
    uint8_t *mem=malloc(bytes); if(!mem)return NULL;
    FILE *fp=fopen(a->path,"rb"); if(!fp){free(mem);return NULL;}
    if(fread(mem,1,bytes,fp)!=bytes){fclose(fp);free(mem);return NULL;} fclose(fp);
    data_cache_hit_writeback(mem,bytes);
    fix39_arena_pixels[header_index]=mem; fix39_arena_pixel_bytes[header_index]=bytes;
    return mem;
}
...
static void fix39_arena_order_init(void){
    ...
    const size_t n=wm_ring_arena_block_count(); if(n!=211)return;
    for(size_t i=0;i<n;i++)fix39_arena_order[i]=(uint16_t)i;
    for(size_t i=1;i<n;i++){
        uint16_t key=fix39_arena_order[i]; size_t j=i;
        const wm_ring_arena_block *kb=wm_ring_arena_block_at(key);
        while(j>0){const wm_ring_arena_block *pb=wm_ring_arena_block_at(fix39_arena_order[j-1]); if(!fix39_arena_draw_before(kb,pb))break; fix39_arena_order[j]=fix39_arena_order[j-1];--j;}
        fix39_arena_order[j]=key;
    }
    fix39_arena_order_ready=true;
}
static void fix39_draw_arena_block(const wm_ring_arena_block *b){
    if(!b)return; const wm_ring_arena_image *img=wm_ring_arena_image_at(b->header_index); const wm_ring_arena_palette *pal=wm_ring_arena_palette_at(b->palette);
    ...
    if(sx+(int)img->width<0||sx>=WM_RING_SOURCE_VIEW_W||sy+(int)img->height<0||sy>=WM_RING_SOURCE_VIEW_H)return;
    const uint8_t *px=fix39_arena_image_load(b->header_index); if(!px)return;
    ...
    int pitch=(img->width+7)&~7,sh=pitch?2048/pitch:1; if(sh<1)sh=1; if(sh>2)sh&=~1;
    const bool fx=(b->flags&WM_BMOD_HFLIP)!=0, fy=(b->flags&WM_BMOD_VFLIP)!=0;
    for(int top=0;top<(int)img->height;top+=sh){int h=(int)img->height-top;if(h>sh)h=sh;int dr=fy?(int)img->height-top-h:top;
...
static void fix39_draw_arena_pass(bool front){
    fix39_arena_order_init(); if(!fix39_arena_order_ready)return;
    for(size_t oi=0;oi<211;oi++){
        const wm_ring_arena_block *b=wm_ring_arena_block_at(fix39_arena_order[oi]); if(!b)continue;
