    ...
    text_line(8, 44, line);
    {
        ...
        if (cs) {
            snprintf(line, sizeof(line), "COLLIS ch:%u ab:%u x:%u y:%u z:%u o:%u r:%u h:%u",
                     cs->combat_checkhit_ticks, cs->combat_attack_boxes_built,
                     cs->combat_x_overlap_ticks, cs->combat_y_overlap_ticks,
                     cs->combat_z_overlap_ticks, cs->combat_full_overlap_ticks,
                     cs->combat_full_overlap_rejected, cs->combat_accepted_hits);
            text_line(8, 56, line);
        }
    }
