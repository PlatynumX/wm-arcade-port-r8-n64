...
static void draw_fighter(const wm_demo_fighter *f) {
    ...
    /* Combat2DT: each source frame owns its exact WIMP palette.  Using the wrestler base-frame TLUT here can reinterpret a valid non-Bret CI8 frame with unrelated colors. */
    ...
    if (fighter_uses_torso_layer(f)) {
        ...
        const wm_source_sprite *torso = NULL;
        if (fix39_runtime_draw_index >= 0) {
            const wm_arcade_actor_t *a=wm_fix39_actor((size_t)fix39_runtime_draw_index);
            const char *tf=wm_fix39_actor_source_torso_frame((size_t)fix39_runtime_draw_index);
            if(a&&tf) torso=wm_character_sprite_find((uint8_t)a->wrestler_num,tf);
        } else if (torso_frame) {
            torso=wm_character_sprite_find(f->roster_id,torso_frame->source_frame);
        }
        if (torso) {
...
static void render_match(const wm_app *app) {
    const wm_demo *demo = &app->demo;
    ...
    /* Shared live source actor renderer for selected matches and
...
static void render_app(const wm_app *app) {
    ...
    if (app->mode == WM_APP_MODE_SELECT) {
        ...
        /* MATCH_INIT owns no live presentation until match_begin succeeds. */
        fill_rect(0, 0, 320, 240, RGBA32(0, 0, 0, 255));
    } else if (app->mode == WM_APP_MODE_MATCH) {
        /* WRESTLE.ASM::start_match live actor/source-frame presentation. */
        render_match(app);
...
static int fix39_frontend_slot_for_arcade(unsigned rid){
