...
/* Combat2BB: CROWD.ASM crowd_anim translation.  BAKLST selects a crowd
...
static bool fix39_draw_crowd_block(const wm_ring_arena_block *b){
    ...
    const fix39_crowd_state *s=&fix39_crowd[ci]; const wm_crowd_asset *a=wm_crowd_asset_at(s->frame); if(!a)return false;
    ...
    uint16_t *pal=NULL; const uint8_t *px=fix39_crowd_load(s->frame,&pal); if(!px||!pal)return true;
    surface_t tex=surface_make_linear((void*)px,FMT_CI8,a->width,a->height);
...
static void draw_ring_back(void) {
    fix39_crowd_tick();
    draw_background();
    /* Exact ascending source OZPOS order below the wrestler object range. */
    fix39_draw_arena_z(100); /* 13c7 mat */
    fix39_draw_arena_z(101); /* 13c8 back posts */
