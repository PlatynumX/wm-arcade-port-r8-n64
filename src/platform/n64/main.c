...
static const uint8_t *fix39_rope_load(const wm_ring_rope_asset *a, const uint16_t **pal) {
    ...
    for (unsigned i=0;i<12;i++) if (fix39_rope_cache_slots[i].a==a) {
        ...
        *pal=(const uint16_t*)(const void*)(fix39_rope_cache_slots[i].mem+a->palette_offset);
    ...
    *pal=(const uint16_t*)(const void*)(c->mem+a->palette_offset); return c->mem;
}
static void fix39_draw_rope_object(WmRopeBank bank,WmRopeChannel ch,WmRopeHalf half) {
    ...
    const uint16_t *pal=NULL; const uint8_t *px=fix39_rope_load(a,&pal); if(!px||!pal)return;
