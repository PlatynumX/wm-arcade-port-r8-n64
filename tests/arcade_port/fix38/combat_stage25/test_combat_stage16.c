...
a.player_mode=WM_PMODE_NORMAL;o.player_mode=WM_PMODE_NORMAL;assert(!wm_arcade_taker_release_charge(&a,&o,109,&c));assert(wm_arcade_taker_release_charge(&a,&o,110,&c));
/* TAKER.ASM:378 #scrt_spirit ends in `movi und_neckbreaker2_anim,a0 /
   calla change_anim1a` -- an ANIMATION, and SPECIAL_MOVE_ADDR is never
   touched on this path. This used to assert special=="scrt_spirit",
   which is the ROUTINE's name: the port called startsp() here, whose
   two seams are empty, so the move fired and did nothing. The five
   siblings in test_combat_stage23 all assert an animation; Taker was
