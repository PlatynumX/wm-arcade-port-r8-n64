...
static wm_arcade_actor_t run_actor(void){wm_arcade_actor_t a;memset(&a,0,sizeof(a));a.player_mode=WM_PMODE_RUNNING;a.move_dir=WM_MOVE_RIGHT;a.facing_dir=WM_MOVE_UP_RIGHT;a.new_facing_dir=WM_MOVE_UP_RIGHT;a.but_val_down=WM_BTN_KICK;return a;}
...
#undef PROVE
    ...
    rt_reset(&u);uc=rt_callbacks(&u);b.yoko=&uc;a1=normal_opp();o1=normal_opp();a1.facing_dir=WM_MOVE_UP_RIGHT;assert(wm_arcade_port_release_charge(&wm_arcade_profile_yoko,&a1,&o1,"charge_salt",85,&b));assert(strcmp(u.anim,"yok_2_salt_anim")==0);b.yoko=NULL;
    ...
    rt_reset(&u);uc=rt_callbacks(&u);b.bam=&uc;a1=normal_opp();o1=normal_opp();a1.facing_dir=WM_MOVE_UP_RIGHT;assert(wm_arcade_port_release_charge(&wm_arcade_profile_bam,&a1,&o1,"firepnch",85,&b));assert(strcmp(u.anim,"bam_2_fpunch_anim")==0);b.bam=NULL;
    rt_reset(&u);uc=rt_callbacks(&u);b.doink=&uc;a1=normal_opp();o1=normal_opp();a1.facing_dir=WM_MOVE_UP_RIGHT;a1.new_facing_dir=WM_MOVE_UP_RIGHT;a1.stick_val_cur=0;assert(wm_arcade_port_release_charge(&wm_arcade_profile_doink,&a1,&o1,"charge_buzz",100,&b));assert(strcmp(u.anim,"dnk_2_buzz_anim")==0);b.doink=NULL;
    rt_reset(&u);uc=rt_callbacks(&u);b.lex=&uc;a1=normal_opp();o1=normal_opp();a1.facing_dir=WM_MOVE_UP_RIGHT;assert(wm_arcade_port_release_charge(&wm_arcade_profile_lex,&a1,&o1,"charge_clobber",100,&b));assert(strcmp(u.anim,"lex_2_clobber_anim")==0);b.lex=NULL;
