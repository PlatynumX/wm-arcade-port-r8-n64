...
static void test_attract_live_cycle(void)
{
    ...
    static const char *const expected_hint_tables[WM_ATTRACT_ACTIVE_HINTS] = {
        "HNT_2", "HNT_4", "HNT_3", "HNT_7", "HNT_5",
        "HNT_8", "HNT_1", "HNT_6", "HNT_9", "HNT_9"
    };
    size_t n;
    size_t demos = 0u;
    ...
    for (size_t i = 0u; i < WM_ATTRACT_ACTIVE_HINTS; ++i) {
        ...
        assert(strcmp(wm_attract_hints[i].body_label, expected_hint_tables[i]) == 0);
...
static void test_v13e_drone_scalar_bindings(void)
{
    ...
    assert(cb->rnd_upto != 0); /* Chunk 6 binds distinct plain RND mask service. */
    ...
    assert(st->drone_scripts_ready == wm_arcade_drone_source_scripts_ready());
    assert(st->drone_scalar_tables_ready == (WM_FIX39_DRONE_SOURCE_GENERATED != 0));
    assert(st->drone_runtime_ready == (st->drone_scalar_tables_ready &&
        st->drone_rndrng0_ready && st->drone_plain_rnd_ready &&
        st->drone_range_tables_ready && st->drone_scripts_ready));
...
static void test_live_collision_frame_bridge(void)
{
    wm_arcade_frame_box_t f0 = { .iani3x = -12, .iani3y = 70, .iani3z = 24, .iani3id = 70 };
    wm_arcade_frame_box_t f1 = { .iani3x = -12, .iani3y = 70, .iani3z = 24, .iani3id = 70 };
    const WmFix39Status *st;

    wm_fix39_runtime_init();
    wm_fix39_match_begin(0u, 1u);
    assert(wm_fix39_match_life(0u) == 100);
    assert(wm_fix39_match_life(1u) == 100);
    assert(!wm_fix39_status()->collision_boxes_ready);
    assert(wm_fix39_match_set_frame_box(0u, &f0));
    assert(!wm_fix39_status()->collision_boxes_ready);
    assert(wm_fix39_match_set_frame_box(1u, &f1));
    st = wm_fix39_status();
    assert(st->collision_boxes_ready);
    assert(st->health_service_ready);
    wm_fix39_match_tick(0, 0, false, false, false, false, false, false);
...
int main(void)
{
    test_live_collision_frame_bridge();
