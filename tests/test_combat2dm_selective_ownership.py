...
spec=importlib.util.spec_from_file_location('apply_fix39_dm',R/'tools/apply_fix39.py'); m=importlib.util.module_from_spec(spec); spec.loader.exec_module(m)
with tempfile.TemporaryDirectory() as td:
    ...
    for n in ['wm_arcade_combat.c','wm_arcade_drone.c','wm_arcade_bret.c','wmania_attract_core.c','wmania_attract_adapter.c','wmania_ring_geometry.c','wmania_rng.c']:
        assert f'src/fix39/{n}' in cm2 and f'src/fix39/{n}' in mk2
        assert f'src/core/arcade/{n}' not in cm2 and f'src/core/arcade/{n}' not in mk2
    for n in ['wm_arcade_roster.c']:
        assert f'src/core/arcade/{n}' in cm2 and f'src/core/arcade/{n}' in mk2
        assert f'src/fix39/{n}' not in cm2 and f'src/fix39/{n}' not in mk2
print('Combat2DM historical regression reconciled to DN dependency-closed ownership: PASS')
