...
def _unassembled_note(label: str) -> str:
    """A banner when the routine this extracts was never assembled.

    The source tree carries plenty of code the assembler skipped, and
    extracting it is fine -- it is real source text -- but a generated
    file that does not say so reads as shipped content. This asks
    port_coverage, which decides it from the `.if` conditions and the
    equates, so the note appears and disappears with the switch rather
    than being a comment someone has to remember to update.
    """
    try:
        sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent))
        import port_coverage
        skipped = port_coverage.unassembled_routines()
    except Exception:
        return ""
    why = skipped.get(label)
    if not why:
        return ""
    return (f"\n   NOT IN THE SHIPPED GAME. {label} sits inside a `.if`\n"
            f"   the assembler evaluated as false ({why}), so the object\n"
            f"   never contained it and nothing in the game can reach it.\n"
            f"   Extracted anyway because it is real source text -- cut\n"
            f"   content, not translated behaviour. Nothing reads this.\n")
