...
def _service_findings(runtime_c: str, findings: list[Finding]) -> None:
    service_checks = [
        ...
        (
            "DO_REVERSAL_MESS",
            "major",
            "reversal_message_not_text_object_verified",
            "DO_REVERSAL_MESS is a source message effect. Routing it through trace/sound metadata is not proof that the arcade message object/timing exists.",
            "Port or connect the real message renderer path and assert timing/object spawn semantics.",
        ),
        (
            "BONUS_MESS_",
            "major",
            ...
            "Translate BONUS_MESS as a real source message process and compare bonus values/timing per wrestler.",
        ),
        (
