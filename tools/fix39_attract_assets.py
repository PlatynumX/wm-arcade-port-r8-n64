...
NAMED_PALETTES = ("RUBYPAL", "WSF_Y_P", "WGFS_W_P", "SGMD8YEL", "SGMD8WHT")
...
def rgba5551(v: int, *, transparent_zero: bool = True) -> int:
    v &= 0x7FFF
...
def resolve_assets(img_dir: Path, wimp) -> dict[str, ResolvedSprite]:
    ...
    needed = {x.upper() for x in HINT_ART}
    ...
    for fn in ("ATTRACT.IMG", "LILLOGO.IMG"):
        try:
    ...
    if needed:
        for p in sorted(img_dir.iterdir(), key=lambda x: x.name.casefold()):
            if not p.is_file() or p.suffix.casefold() != ".img":
                continue
            if p.name.casefold() in {x.casefold() for x in FONT_CONTAINERS} | {"attract.img", "lillogo.img"}:
                continue
            try:
                add_required_from_container(wimp, p, needed, resolved)
            ...
            if not needed:
                break
    if needed:
        raise ValueError("missing required attract source images: " + ", ".join(sorted(needed)))
    return resolved
...
def emit_c(path: Path, header_name: str, resolved: dict[str, ResolvedSprite],
    ...
    wanted_keys = sorted(font_keys | {x.upper() for x in HINT_ART})
    ...
    for name in NAMED_PALETTES:
        vals = [rgba5551(v) for v in named_pals[name]]
    ...
    for key in wanted_keys:
        ...
        if r.palette is not None:
            ...
            fallback = FONT_DRAW_PALETTE.get(r.container.name.upper())
    ...
    lines += [
        ...
        "        if (fix39_source_name_equal(sprites[i].source_frame, source_frame)) return &sprites[i];",
        "    return 0;",
        "}", "",
        "size_t wm_fix39_attract_sprite_count(void) {",
...
def self_test() -> None:
    ...
    print("Fix39 V12d attract asset parser self-test: PASS")
