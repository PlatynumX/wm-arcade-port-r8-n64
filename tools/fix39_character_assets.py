...
def emit(root,out_c,out_h,out_fs=None):
    ...
    for rid,name,pfx in CHARS:
        ...
        for frame in frames:
            ...
            print(f'fix39_character_assets: {name}:{frame} palette={pal.name} colors={pal.color_count} base={pal_base} mode={pal_mode} raw_ci8={min(raw_px) if raw_px else 0}..{max(raw_px) if raw_px else 0} ci8={min(pxvals) if pxvals else 0}..{max(pxvals) if pxvals else 0}')
