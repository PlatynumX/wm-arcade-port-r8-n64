...
def translate_local_cfg(lines):
    ...
    for s in src:
        if re.match(r'^#[A-Za-z_][A-Za-z0-9_]*:?$',s) or re.match(r'^[A-Za-z_][A-Za-z0-9_]*:$',s):
            labels[_lname(s)]=1
    if not labels and not any(re.match(r'(?i)^jr(?:uc|z|nz|eq|ne|lt|le|gt|ge)\b',s) for s in src):
        return None
    ...
    for s in src:
        ...
        return None
    if not (saw_ret and saw_branch): return None
    out.append('    return 0;')
    return out
...
def self_test():
    with tempfile.TemporaryDirectory() as td:
        ...
        r.write_text(
            ...
            '  subk 1,a0\n  move a0,*a13(DRN_DELAY)\n  rets\n  #zero\n  clr a0\n  move a0,*a13(DRN_BUT)\n  rets\n'
            'body=bar|x|addr=3|status=source-lines|lines=2\n  callr rnd\n  rets\n')
        n=emit(r,o); t=o.read_text(); assert n==3 and 'drone->delay' in t and 'foo@EXGPC_0000' in t and '"bar"' not in t
