...
NATIVE_15 = [
    ("drn_seek@EXGPC_0000", "wm_fix39_native_drn_seek"),
    ("drn_retreat@EXGPC_0000", "wm_fix39_native_drn_retreat"),
    ("drone_chrg", "wm_fix39_native_drone_chrg"),
    ("drn_climbtb@EXGPC_0000", "wm_fix39_native_drn_climbtb"),
    ("drn_taunt@EXGPC_0000", "wm_fix39_native_drn_taunt"),
    ("drn_enterring@EXGPC_0000", "wm_fix39_native_drn_enterring"),
    ("drn_opinair@EXGPC_0000", "wm_fix39_native_drn_opinair"),
    ("drn_oprun@EXGPC_0000", "wm_fix39_native_drn_oprun"),
    ("drn_roll@EXGPC_0000", "wm_fix39_native_drn_roll"),
    ("drn_inair@EXGPC_0000", "wm_fix39_native_drn_inair"),
    ("drn_ontb@EXGPC_0000", "wm_fix39_native_drn_ontb"),
    ("drn_run@EXGPC_0000", "wm_fix39_native_drn_run"),
    ("drn_combo@EXGPC_0000", "wm_fix39_native_drn_combo"),
    ("drn_seekclose@EXGPC_0000", "wm_fix39_native_drn_seekclose"),
    ("drn_oppdead@EXGPC_0000", "wm_fix39_native_drn_oppdead"),
]

def emit_native_15(blocks, out:Path):
    by_label = {label: lines for label, lines in blocks}
    expected = [label for label, _fn in NATIVE_15]
    if not all(label in by_label and by_label[label] for label in expected):
        return None

    # Do not call the native set complete unless the recovery pass found all
    # fifteen exact source windows.  The C implementations are explicit ports,
    # while this generated header is only the source-to-handler binding.
    q = [
        '#ifndef WM_ARCADE_DRONE_SOURCE_BODIES_GENERATED_H',
        '#define WM_ARCADE_DRONE_SOURCE_BODIES_GENERATED_H',
        '#define WM_FIX39_DRONE_TRANSLATED_BODY_COUNT 15',
        'static const WmFix39DroneGeneratedBody wm_fix39_generated_bodies[15] = {',
    ]
    for label, fn in NATIVE_15:
        q.append(f'    {{"{label}", {fn}}},')
    q += ['};', '#endif']
    out.write_text('\n'.join(q) + '\n', encoding='utf-8')
    return 15
...
def emit(report:Path,out:Path):
    ...
    native_count=emit_native_15(blocks,out)
...
def self_test():
    with tempfile.TemporaryDirectory() as td:
        ...
        rn=d/'native_report'; on=d/'native.h'
        lines=[]
        for label,_fn in NATIVE_15:
            lines.append(f'body={label}|x|addr=0|status=source-lines|lines=1')
            lines.append('  rets')
        rn.write_text('\n'.join(lines)+'\n')
        nn=emit(rn,on); nt=on.read_text()
        assert nn==15 and 'WM_FIX39_DRONE_TRANSLATED_BODY_COUNT 15' in nt
        assert 'wm_fix39_native_drn_combo' in nt and 'wm_fix39_native_drone_chrg' in nt
