...
def translate(lines):
    ...
    for raw in lines:
        ...
        if m:
            ...
            regs[r]=regs[r]+v if m.group(1).lower()=='addk' else regs[r]-v; continue
...
def emit(report:Path,out:Path):
    ...
    for label,ops in translated:
        ...
        for field,val in ops:
            ...
            q.append(f'    drone->{field}=({cast})({val});')
        q.append('    return 1; }')
...
def self_test():
    with tempfile.TemporaryDirectory() as td:
        ...
        r.write_text('body=foo@EXGPC_0000|x|addr=0|status=source-lines|lines=3\n  movk 7,a0\n  move a0,*a13(DRN_DELAY)\n  rets\nbody=bar|x|addr=1|status=source-lines|lines=2\n  callr rnd\n  rets\n')
        n=emit(r,o); t=o.read_text(); assert n==1 and 'drone->delay' in t and 'foo@EXGPC_0000' in t and 'bar' not in t
