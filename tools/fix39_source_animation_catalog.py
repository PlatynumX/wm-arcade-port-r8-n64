...
def visual_seq(entry,owner_prefix,eval_expr):
    ...
    return p.name,frames,repeat,init
...
def emit(root,out_c,out_h,bret_h,razor_h):
    ...
    for rid,name,pfx in CHARS:
        for lab,e in idx.items():
            ...
            src,fr,rep,init=visual_seq(e,prefix_first[pfx],eval_expr)
            if fr:defs.append((rid,lab,src,fr,rep,init))
    ...
    for n,(rid,lab,src,fr,rep,ini) in enumerate(defs):
        s=f'afr_{n}';d=f'ad_{n}';syms.append(d);c.append(f'static const wm_source_anim_frame_t {s}[]={{')
        for nm,tk in fr:c.append(f'{{"{nm}",{tk}u}},')
        c.append('};');c.append(f'static const wm_source_anim_def_t {d}={{{rid}u,"{lab}","{src}",{s},{len(fr)}u,{1 if rep else 0}u,{{{ini["mask"]}u,{ini["anim_mode"]}u,{ini["player_mode"]}u,{ini["speed"]}u,{ini["friction"]},{ini["xvel"]},{ini["yvel"]},{ini["zvel"]},{ini["gravity"]}}}}};')
