...
# Per-wrestler `.long animation` tables that an ANI_CODE routine reads
# directly. Unlike the four above, no opcode names these -- the routine
# that indexes one is native code, so the site is named here. Only the
# site: the nine rows are still read out of the source, and a table that
# stopped being nine rows of animation labels would fail the same
# ROSTER_SLOTS check the slave tables get.
CODE_TABLES = (
...
def code_tables() -> dict[str, list[str]]:
    ...
    out: dict[str, list[str]] = {}
    for fname, label, routine in CODE_TABLES:
        ...
        names: list[str] = []
        j = at + 1
        while len(names) < ROSTER_SLOTS and j < len(lines):
        ...
        if len(names) < ROSTER_SLOTS:
            raise ValueError(
                f"{fname}:{at + 1}: {label} has {len(names)} entries, "
                f"not {ROSTER_SLOTS}")
        out[routine] = names[:ROSTER_SLOTS]
    return out
...
def render_aux_c() -> str:
    ...
    code = code_tables()
    if code:
        out.append("/* Per-wrestler tables an ANI_CODE routine indexes"
                   " directly. */")
        out.append("static const char *const code_rows"
                   "[][WM_ANIM_ROSTER_SLOTS] = {")
        for routine in sorted(code):
            cells = ", ".join("0" if not n else '"%s"' % n
                              for n in code[routine])
            out.append("    { %s },   /* %s */" % (cells, routine))
        out += ["};", ""]
        out.append("static const char *const code_names[] = {")
        for routine in sorted(code):
        ...
        out += [
            ...
            "    for (i = 0; i < sizeof(code_names) / sizeof(code_names[0]);",
            "         ++i)",
