...
# REACT5.ASM:333 bncoff and :353 bncoff_gate -- the animation a wrestler
# plays when he bounces off something, one pair per roster slot (facing
# 2 and facing 4). Ten rows of two, indexed [WRESTLERNUM][facing], and
# unlike the three tables above these are shared globals rather than one
# table per wrestler.
#
# bncoff_gate is the same list with Yokozuna's two entries swapped for
# his *_bncoff_gate_anim pair -- he is the only wrestler with a
# different animation for bouncing off the gate.
PAIR_TABLES = {
    "bncoff": "REACT5.ASM",
    "bncoff_gate": "REACT5.ASM",
}


def pair_table(name: str) -> list[list[str] | None]:
    """A [roster slot][facing] table of animation labels."""
    path = wlanim.ORIG / PAIR_TABLES[name]
    lines = assembled_lines(path)
    at = next((i for i, line in enumerate(lines)
               if (m := wlanim.SUBR_RE.match(line)) and m.group(1) == name),
              None)
    if at is None:
        raise ValueError(f"{path.name}: no {name}")
    out: list[list[str] | None] = []
    for i in range(at + 1, len(lines)):
        line = lines[i]
        if not line.strip():
            continue
        if re.match(r"^\s*\.ref\b", line, re.I):
            continue            # the declarations interleaved with the rows
        m = LONG_RE.match(line)
        if not m:
            break
        parts = [p.strip() for p in m.group(1).split(",")]
        if len(parts) != 2:
            raise ValueError(f"{name}: row is not a facing pair: {parts}")
        for p in parts:
            if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", p):
                raise ValueError(f"{name}: unreadable entry {p!r}")
        out.append(parts)
        if len(out) == ROSTER_SLOTS:
            break
    if len(out) != ROSTER_SLOTS:
        raise ValueError(f"{name}: {len(out)} rows, expected {ROSTER_SLOTS}")
    return out
...
def render_c() -> str:
    ...
    for tname in sorted(PAIR_TABLES):
        grid = pair_table(tname)
        out.append(f"/* REACT5.ASM {tname} -- [roster slot][facing 2, 4] */")
        out.append(f"const char *const wm_wrestler_{tname}"
                   f"[WM_WRESTLER_ANIM_SLOTS][2] = {{")
